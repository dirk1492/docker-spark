...
spark_validate() {
    ...
    # Validate thriftserver node inputs
    if [[ "$SPARK_MODE" == "thriftserver" ]]; then
        if [[ -z "$SPARK_MASTER_URL" ]]; then
            print_validation_error "For thriftserver nodes you need to specify the SPARK_MASTER_URL"
        fi
    fi
...
spark_generate_conf_file() {
    if [ ! -z "$SPARK_DRIVER_HOST" ]; then
        info "Set spark.driver.bindAddress = 0.0.0.0"
        info "Set spark.driver.host = $SPARK_DRIVER_HOST"
        echo "spark.driver.bindAddress    0.0.0.0" >> "${SPARK_CONFDIR}/spark-defaults.conf.template"
